...
function toUserFacingError(error: { code?: string; message: string }): Error {
  if (error.code === '23514') {
    ...
    if (error.message.includes('events_ends_after_starts')) {
      eventDebugLog('Supabase DB 실패 [SUPABASE-23514 events_ends_after_starts]', {
        code: error.code,
        dbMessage: error.message,
      })
      return new Error(
...
function resolveAllDay(event: Pick<CalendarEvent, 'start' | 'allDay'>): boolean {
...
function eventToInsertRow(
  ...
  if (!allDay && endsAt && event.end) {
    ...
    if (endMs <= startMs) {
      const { date: startDate, time: startTime } = splitLocalDateTime(event.start)
      ...
      if (endDate === startDate) {
        ...
        const correctedEnd = buildEventDateTime(startDate, correctedTime, false)
        const correctedMs = new Date(parseEventDate(correctedEnd, false)).getTime()
